...
tool_selector_system_prompt = f"""
    ...
    * register_routine_by_prescription * 
     register_prescription_routine: 처방전을 통한 복용 일정 등록 (예: "처방전으로 복용 일정 등록", "처방전 복약 일정", "병원에서 받은 처방전으로 등록")
    * '처방전'이라는 키워드가 명확히 언급되어야 함
     
    pill_photo_routine: 알약 촬영을 통한 복용 일정 등록 또는 정보 조회(예: "알약 사진 찍어서 등록", "약 촬영해서 일정 만들어줘", "이 약이 무엇일까?")
